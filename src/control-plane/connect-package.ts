...
function exactFipsOrigin(value: string, transportNpub: string): string {
  const endpoint = new URL(value);
  if (endpoint.protocol !== "http:" || endpoint.hostname !== `${transportNpub}.fips` || !endpoint.port
    || endpoint.username || endpoint.password || endpoint.pathname !== "/" || endpoint.search || endpoint.hash) {
    throw new Error("Connect package FIPS endpoint must match its signed transport identity exactly");
  }
...
export function canonicalConnectManifest(manifest: AutopilotConnectManifestV1 | AutopilotConnectManifestV2): string {
  return canonicalValue(manifest);
}

function manifestApi(ownerNpub: string): AutopilotConnectManifestV1["api"] {
  const ownerSegment = encodeURIComponent(ownerNpub);
  return {
...
export function createAutopilotConnectPackage(input: {
  ...
  const manifest: AutopilotConnectManifestV1 = {
    ...
    api: manifestApi(input.ownerNpub),
  };
...
export function createAutopilotConnectPackageV2(input: {
  identity: WingmanInstanceIdentity;
  fipsEndpoint: string;
  fipsNodeNpub: string;
  ownerNpub: string;
  ...
  const now = input.now ?? new Date();
  const installationId = installationIdForIdentity(input.identity);
  const manifest: AutopilotConnectManifestV2 = {
    kind: CONNECT_PACKAGE_KIND,
    version: CONNECT_PACKAGE_TRANSPORT_VERSION,
    generated_at: now.toISOString(),
    installation: { id: installationId, npub: input.identity.npub },
    transport: { fips: { npub: input.fipsNodeNpub } },
    endpoints: {
      fips: exactFipsOrigin(input.fipsEndpoint, input.fipsNodeNpub),
      https: input.httpsEndpoint ? new URL(input.httpsEndpoint).origin : null,
    },
    api: manifestApi(input.ownerNpub),
  };
...
export function verifyAutopilotConnectPackage(
...
): AutopilotConnectPackageV1 | AutopilotConnectPackageV2 {
  ...
  const candidate = input as unknown as AutopilotConnectPackageV1 | AutopilotConnectPackageV2;
  const { manifest, signature } = candidate;
  if (manifest.kind !== CONNECT_PACKAGE_KIND) throw new Error("Unsupported connect package kind");
  if (manifest.version !== CONNECT_PACKAGE_VERSION && manifest.version !== CONNECT_PACKAGE_TRANSPORT_VERSION) {
    throw new Error("Unsupported connect package version");
  }
  ...
  const transportNpub = manifest.version === CONNECT_PACKAGE_TRANSPORT_VERSION
    ? manifest.transport?.fips?.npub
    : manifest.installation.npub;
  if (typeof transportNpub !== "string") throw new Error("Connect package FIPS transport identity is invalid");
  try {
    const decoded = nip19.decode(transportNpub);
    if (decoded.type !== "npub" || typeof decoded.data !== "string") throw new Error("not npub");
  } catch {
    throw new Error("Connect package FIPS transport identity is invalid");
  }
  exactFipsOrigin(manifest.endpoints.fips, transportNpub);
